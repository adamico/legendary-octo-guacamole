...
function Combat.projectile_fire(entity)
   ...
   if wants_to_shoot and has_enough_hp and cooldown_ready then
      ...
      local dir = EntityUtils.get_direction_name(sx, sy)
      ...
      -- Center projectile on shooter using hitboxes
      local shooter_hb = HitboxUtils.get_hitbox(entity)
      local proj_config = GameConstants.Projectile.Laser
      local proj_w = proj_config.width
      local proj_h = proj_config.height

      local spawn_x = shooter_hb.x + (shooter_hb.w / 2) - (proj_w / 2)
      local spawn_y = shooter_hb.y + (shooter_hb.h / 2) - (proj_h / 2)
